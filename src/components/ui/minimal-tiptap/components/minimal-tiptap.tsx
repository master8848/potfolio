...
const MinimalTiptapEditor = forwardRef<HTMLDivElement, MinimalTiptapProps>(
  ({ value, outputValue = 'html', disabled, contentClass, onValueChange, className, speech, ...props }, ref) => {
    ...
    const setBlog = useCallback((text: string) => {
    ...
    return (
      <div>
        ...
        {speech && browserSupportsSpeechRecognition ? <div className="">
          ...
          <div className=" space-x-2 my-2">
            ...
            <Button type='button' onClick={() => { setBlog(transcript); resetTranscript() }}>Add to Blog</Button>
          </div>
        </div> : null}</div>
    )
  }
