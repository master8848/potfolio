...
import { text, date, varchar, timestamp, pgTable, uniqueIndex } from "drizzle-orm/pg-core";
...
export const insertBlogSchema = createInsertSchema(blogs).omit(timestamps).omit({
  id: true,
  userId: true
});
export const insertBlogParams = baseSchema.extend({
